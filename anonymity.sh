...
function newnym() {
  ...
  (
    # Security: Escape backslashes, double quotes, and remove newlines to prevent protocol injection
    local ESCAPED_PASS="${AUTH_PASSWORD//\\/\\\\}"
    ESCAPED_PASS="${ESCAPED_PASS//\"/\\\"}"
    ESCAPED_PASS="${ESCAPED_PASS//$'\r'/}"
    ESCAPED_PASS="${ESCAPED_PASS//$'\n'/}"

    # Send commands to Tor Control Port via netcat
    # -w 5 sets a 5 second timeout
    # Use printf for reliable string formatting and CRLF (\r\n) handling
