...
function interactive_menu() {
  while true; do
	...
	4|e|E) enable_all ;;
	5|o|O)
        echo -e "${YELLOW}⚠️  WARNING: This will expose your real IP!${RESET}"
        read -r -p "Are you sure you want to disable anonymity? [y/N] " confirm
        if [[ "$confirm" =~ ^[yY][eE]?[sS]?$ ]]; then
            disable_all
        else
            echo -e "${GREEN}Action cancelled. Stay safe!${RESET}"
        fi
        ;;
	6|r|R) restart_all ;;
