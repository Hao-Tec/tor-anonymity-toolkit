...
# Global IP Checkers (Optimization: The fastest/working checker will swap to index 0)
IP_CHECKERS=(
  "https://ident.me"
  ...
  if nc -z -w3 127.0.0.1 9050; then
    ...
    for i in "${!IP_CHECKERS[@]}"; do
      ...
      if [[ $TOR_IP =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        ...
        # Optimization: Move successful checker to front of list
        if [[ $i -gt 0 ]]; then
            temp="${IP_CHECKERS[0]}"
            IP_CHECKERS[0]="$url"
            IP_CHECKERS[$i]="$temp"
            debug_log "Optimized: '$url' promoted to primary IP checker."
        fi
        break
      else
