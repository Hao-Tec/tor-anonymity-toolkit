...
function newnym() {
  ...
  (
    ...
    # Security: Escape password to prevent protocol injection and use printf instead of echo -e
    local safe_pw="${AUTH_PASSWORD//\\/\\\\}"
    safe_pw="${safe_pw//\"/\\\"}"
    output=$(printf "AUTHENTICATE \"%s\"\r\nSIGNAL NEWNYM\r\nQUIT\r\n" "$safe_pw" | nc -w 5 localhost $CONTROL_PORT 2>&1)
