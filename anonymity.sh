...
function newnym() {
  ...
  (
    # Security: Sanitize password to prevent command injection
    # 1. Escape backslashes
    # 2. Escape double quotes
    local sanitized_pw="${AUTH_PASSWORD//\\/\\\\}"
    sanitized_pw="${sanitized_pw//\"/\\\"}"

    # Send commands to Tor Control Port via netcat
    # -w 5 sets a 5 second timeout
    # Use CRLF (\r\n) as per Tor Control Protocol spec
    output=$(printf "AUTHENTICATE \"%s\"\r\nSIGNAL NEWNYM\r\nQUIT\r\n" "$sanitized_pw" | nc -w 5 localhost $CONTROL_PORT 2>&1)
