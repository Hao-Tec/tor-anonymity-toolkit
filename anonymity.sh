...
function newnym() {
  ...
  (
    ...
    # Security: Escape backslashes and double quotes for Tor Control Protocol
    local safe_password="${AUTH_PASSWORD//\\/\\\\}"
    safe_password="${safe_password//\"/\\\"}"

    # Use printf for safe command construction, avoiding echo -e's interpretation of the password content
