...
# This prevents privilege escalation if the file was world-writable
if [[ -f "$CONFIG_FILE" ]]; then
  current_perms=$(stat -c "%a" "$CONFIG_FILE" 2>/dev/null)
  if [[ "$current_perms" != "600" ]]; then
    echo -e "${YELLOW}⚠ Insecure permissions detected on config file ($current_perms). Fixing...${RESET}"
