...
function newnym() {
  ...
  (
    # Security: Escape backslashes and double quotes in password to prevent protocol injection
    local safe_password="${AUTH_PASSWORD//\\/\\\\}"
    safe_password="${safe_password//\"/\\\"}"

    # Send commands to Tor Control Port via netcat
    # -w 5 sets a 5 second timeout
    # Use printf for reliable CRLF (\r\n) handling and to avoid echo -e pitfalls
    output=$(printf "AUTHENTICATE \"%s\"\r\nSIGNAL NEWNYM\r\nQUIT\r\n" "$safe_password" | nc -w 5 localhost $CONTROL_PORT 2>&1)
