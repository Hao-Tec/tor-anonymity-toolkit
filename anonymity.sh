...
function newnym() {
  ...
  # Security: Sanitize AUTH_PASSWORD to prevent protocol injection
  # Check for control characters (newlines, carriage returns)
  if [[ "$AUTH_PASSWORD" =~ [$'\r\n'] ]]; then
      echo -e "${RED}Error: AUTH_PASSWORD contains invalid characters (newlines).${RESET}"
      return 1
  fi
  ...
  (
    ...
    # Use printf to properly format CRLF (\r\n) and avoid shell injection from password variables
