...
function interactive_menu() {
  while true; do
    ...
    echo -e "${GREEN}  1) Toggle Tor + NEWNYM     [t]"
    echo -e "  2) Show Status             [s]"
    echo -e "  3) Send NEWNYM Signal      [n]"
    echo -e "  4) Enable Tor + NEWNYM     [e]"
    echo -e "  5) Disable Tor + NEWNYM    [x]"
    echo -e "  6) Restart Both Services   [r]"
    echo -e "  7) Check if Traffic is Tor [c]"
    echo -e "  8) Monitor Tor IP (Live)   [m]"
    echo -e "  9) Show Dashboard          [d]"
    echo -e " 10) Show Help               [h]"
    echo -e " 11) Exit                    [q]"
    echo
    echo -e "${CYAN}💡 ProTip:${RESET} Use ${YELLOW}monitor${RESET} to watch your IP change in real-time."
    echo
    read -p "$(echo -e "${YELLOW}Choose option or shortcut: ${RESET}")" choice
	...
	*) echo -e "${RED}Invalid option. Please choose between 1-11 or use a shortcut.${RESET}" ;;
     esac
